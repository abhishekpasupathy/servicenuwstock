  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        <QuoteCard label="Volume" value={(displayQuote?.volume ?? 0).toLocaleString()} />
      ...
      <div className="grid grid-cols-[minmax(0,7fr)_minmax(280px,3fr)] gap-4">
        ...
        <aside className="space-y-4">
          ...
          <div className="terminal-card grid grid-cols-2 gap-3 p-4 text-sm"><span>P/E {quote?.pe_ratio?.toFixed(1) ?? "--"}</span><span>Market Cap {money((quote?.market_cap ?? 0) / 1e9)}B</span><span>Avg Vol {(quote?.avg_volume ?? 0).toLocaleString()}</span><span>Source {liveQuote?.provider ?? quote?.source}</span></div>
        </aside>
      </div>
